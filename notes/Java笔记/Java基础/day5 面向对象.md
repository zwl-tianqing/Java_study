...
> <font color='red'>面向对象</font>是一种编程思想，它将我们生活中的实体，将它的相关`属性`和`功能（行为）`定义成了<font color='red'>概念事件的类</font>，利用这个类去解决我们需要解决的问题。

* 其实面向对象（opp）是相对**面向过程**（pop）而言的。
  
  * 两者都是一种思想
  
  * 面向过程，<font color='red'>强调的是功能行为,已函数为最小单位，考虑怎么做</font>
  
  * 面向对象，将功能封装进对象，<font color='red'>强调具备功能的类，以类/对象为最小单位，考虑谁来做。</font>
...
* 属性分类
  ...
  * 成员变量
    ...
    > - 成员变量定义在类中，方法外。在整个类中都能访问。
    > 
    > - 成员变量分为类成员变量和实例成员变量，实例变量存在于对象所在的堆中。
    > 
    > - 成员变量有默认初始值。
    > 
    > - 成员变量的权限修饰符可以根据需要需要，任选一个。
    > 
    > - 类成员变量生命周期在类不被加载时结束。
    > 
    > - 实例成员变量生命周期在对象不被使用时结束。
  - 局部变量：
    
    > - 局部成员变量只定义在局部范围类，如：代码块内，方法内等。
    > - 局部变量只存在与栈内存中。
    > - 作用的范围结束，变量空间自动释放。
    > - 局部变量没有默认值，每次使用必须显示初始化。
    > - 局部变量声明时不指定权限修饰符
...
2. 方法的声明格式
   ...
   > 
   > - 说明：修饰符：public，private，protected等。返回值类型：return 语句传递返回值。没有返回值：void。
...
3. 如何使用使用方法
   ...
   > - 方法中内只能调用方法，不能定义方法
   > 
   > - 同一个类中，所有的方法可以直接调用，不需要new创建对象（当然 main 方法不行，只能调用静态的方法）
   > 
   > - PS：方法中使用匿名内部类书写的方法不属于在方法中定义方法，而是在类中定义方法

4. 方法的重载
   
   > - 概念：在同一个类中，允许存在一个的同名方法，只要它们的参数个数和参数类型不同即可。
   > 
   > - 重载的特点：与返回类型无关，只看参数列表，且参数列表必须不同。（参数个数或参数类型）调用时，根据方法参数列表的不同来区别。
...
1. 构造器的特征
   ...
   > - 不能被static、final、synchronized、native修饰，不能有return语句返回值
...
7. **关键字——this**
   ...
   * **注意：**
     ...
     > **1.使用this()必须放在构造器的首行(也就意味着一个构造器里面最多只使用一个this()调用其他构造器)**
     > 
     > **2.使用this调用本类其他构造器时，必须保证至少有一个构造器不是用this()（实际上就是不能出现构造器自己调用自己）**
...
> 
> - 隐藏一个类中不需要对外提供的实现细节；
> - 使用者通过实现制定好的方法来访问数据，可以方便的加入控制逻辑，限制对属性的不合理操作；
> - 便于代码的修改，增强代码的可维护性；
...
3. **继承的作用**
   ...
   * 提高代码的复用性
   ...
   - **注意：不要仅为了获取其它类中的某个功能去继承类。继承是有逻辑关系的，不要随便继承。**
   
   - 子类继承父类，就继承了父类的属性和方法（但不包括构造方法、私有属性和私有方法）
...
4. 继承的表现
   ...
   * **方法重写**
   ...
   - 要求：
     ...
     > 1. 重写方法必须和被重写的方法具有相同的方法名称、参数列表、返回值类型。
     > 
     > 2. 重写方法不能比被重写的方法的权限还有严格（比如：父类public的方法，子类重写后不能用private）
     > 
     > 3. 重写的方法和被重写的方法要么就是static的，或者就是非static
     > 
     > 4. 子类方法抛出的异常不能大于父类被重写方法的异常

5. <font color='red'>**关键字super**</font>
   ...
   * **this与super的区别**
     ...
     PS：在子类中，通过**this**或**super**调用构造器只能出现一个，因为它们都要占据构造器首行
   
   **类对象的实例化过程**
   
   ![捕获PNG](https://i.loli.net/2020/09/28/HG4iJSvatxTP1yF.png)
   
   **子类对象的实例化过程**
   
   PS：子类实例化的过程中先加载父类，在加载子类
